%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Color Segmentation using GMM for ESE 650 Project 1
...
for i = 1:length(TestImgs)
    ...
    PXGivenCMask = PXGivenC>=0.015*max(max(PXGivenC));% & PXGivenC >= 100
    ...
    for q = 1:size(OBB,1)
        ...
        if(~(min(OBB(q,3:4))./max(OBB(q,3:4))<=0.84 && min(OBB(q,3:4))./max(OBB(q,3:4))>=0.47))
            OBB(q,6) = 1;
            for w = 1:NumSeg
    ...
    IDisp = PlotMaskOnRGB(IRGB, MaskFull,[0,1,0]);
    ...
    for q = 1:NumBarrels(i)
        if(OBB(q,6)==0)
            ...
            text(OBB(q,1)+50, OBB(q,2), ...
                ...
                , num2str(OBB(q,5))]),'FontSize',12,'Color','r');
        end
    end
    toc
